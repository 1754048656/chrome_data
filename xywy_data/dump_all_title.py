...
class DumpAllTitle:
    ...
    def __init__(self):
        ...
        self.test()
        #print(var.server_num)


    def getAllQADetail(self, subject_num, res_name):
        for i in range(self.num, 500):
            try:
                self.headers['User-Agent'] = self.uaList[random.randint(0, len(self.uaList) - 1)]
                print(self.headers['User-Agent'])
                full_url = self.baseUrl + '/list_' + str(subject_num) + '_all_' + str(i) + '.htm'
                print('full_url => ' + full_url)
                response = requests.get(full_url, cookies=self.cookies, headers=self.headers, timeout=8)
                response.encoding = 'gbk'
                # <div class="replay-content-box clearfix">(.*?)</div>
                # print(response.text)
                questions_text = response.text
                # <span class="q fl">Q:</span><a class="fl th" href="/wenda/196408249.htm" target="_blank">隆鼻适合哪些人群</a>
                # <span class="q fl">Q:</span><a class="fl th" href="(.*?)</a>
                # res = re.compile('<span class=\"q fl\">Q:</span><a class=\"fl th\" href=\"(.*?)\"', re.S).findall(questions_text)

                qa_name = re.compile('<span class=\"q fl\">Q:</span><a class=\"fl th\" href=\"(.*?)</a>', re.S).findall(
                    questions_text)

                qa_title = ""
                qa_list = []
                for res_item in qa_name:
                    res_href = res_item.split('\" target=\"_blank\">')[0]
                    qa_title = res_item.split('\" target=\"_blank\">')[1]
                    res_tmp = res_name + str(i) + ' => ' + qa_title + ' -> ' + self.baseUrl + res_href + '\n'
                    print(res_tmp)
                    qa_list.append(res_tmp)

                if qa_title in self.tmpList:
                    ...
                    self.tmpNum = 0
                    self.tmpList.append(qa_title)
                    for qa_list_item in qa_list:
                        self.write2File('href_QA_detail.txt', qa_list_item)
                time.sleep(0.1)
    ...
    def test(self):
        file = open("href_" + str(var.server_num) + ".txt", 'r', encoding='utf-8')
        all_text = file.read()
        # href="/small_680.htm">脑萎缩</a>
        # href="(.*?)"</a>
        res = re.compile('href=\"(.*?)</a>', re.S).findall(all_text)
        for res_item in res:
            res_url = self.baseUrl + res_item.split('\">')[0]
            res_name = res_item.split('\">')[1]
            print(res_url)
            subject_num = re.compile('small_(.*?)\.htm', re.S).findall(res_url)[0]
            print(subject_num)
            self.getAllQADetail(subject_num, res_name)
