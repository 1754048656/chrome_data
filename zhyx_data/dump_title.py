...
class DumpTitle:
    def __init__(self):
        ...
        file = open("dump_title.txt", "r", encoding="utf-8")
        data = file.read()
        json_tmp = json.loads(data)
        info_list = json_tmp['data']['result']['infos']
        for info_list_item in info_list:
            artDropTitle = info_list_item['artDropTitle']
            artUrl = info_list_item['artUrl']
            print('-------------------------')
            print('artDropTitle => ' + artDropTitle)
            print('artUrl => ' + artUrl)
